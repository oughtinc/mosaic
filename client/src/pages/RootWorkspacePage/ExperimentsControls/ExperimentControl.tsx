    ...
    return (
      <FormGroup controlId="formControlsSelect">
        {!this.state.isShowingEditNameField ? (
          <span>
            <Link to={`/e/${experiment.id}`}>
              <ControlLabel style={{ cursor: "pointer" }}>
                {experiment.name}
              </ControlLabel>
            </Link>
            <span onClick={this.showEditNameField} style={wordLinkStyle}>
              edit name
            </span>
          </span>
        ) : (
          <span>
            ...
            />
            <span onClick={this.updateExperimentName} style={wordLinkStyle}>
              save
            </span>
            <span onClick={this.cancelEditingName} style={wordLinkStyle}>
              cancel
            </span>
          </span>
        )}
        <ToggleButtonGroup
          ...
          name="options"
          value={
            experiment.eligibilityRank === null ? 0 : experiment.eligibilityRank
          }
          onChange={value =>
            this.props.onEligibilityRankChange(experiment.id, value)
          }
        >
        ...
        <ToggleButtonGroup
          ...
          onChange={value =>
            this.props.onDefaultOracleChange(
              experiment.id,
              value === 1 ? true : false,
            )
          }
