...
HyperTextArray = (props) => {
  return (
    <div className="HyperTextArray">
      {props.value.map((x, i) => <HyperText key={i} value={x} links={props.links} store={props.store} />)}
    </div>);  
};
...
const Node: React.SFC<NodeProps> = (props) => {
  return <HyperText value={props.node.content} links={props.node.links} store={props.store} />;
};
