...
export default async function sendEmailNotification(
  ...
  if (!process.env.SENDGRID_API_KEY) {
    console.log(
      `DUMMY: Would e-mail ${notificationRequest.user.email} about ${
  ...
  await SendGrid.send({
    to: {
      name: `${notificationRequest.user.givenName} ${
        notificationRequest.user.familyName
      }`,
      email: notificationRequest.user.email,
    },
    from: "Mosaic <mosaic@ought.org>",
    subject: "A new Mosaic workspace is available",
    text: `A new workspace is available in the experiment "${
      notificationRequest.experiment.name
