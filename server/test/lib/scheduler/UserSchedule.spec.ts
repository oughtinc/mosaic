...
import { USER_ID, rootParentCacheFake, workspaces } from "./utils";
...
describe("UserSchedule class", function() {
  ...
  describe("hasUserWorkedOnWorkspace method", function() {
    context(
      "with a user that has not been assigned to any workspace",
      function() {
        it("returns false", function() {
          expect(
            this.userSchedule.hasUserWorkedOnWorkspace(workspaces[0]),
          ).to.equal(false);
        });
      },
    );
    ...
    context(
      "with a user that has been assigned to a workspace once",
      function() {
        ...
        it("returns false for other workspace", function() {
          this.userSchedule.assignWorkspace(workspaces[1]);
          expect(
            this.userSchedule.hasUserWorkedOnWorkspace(workspaces[2]),
          ).to.equal(false);
        });
      },
    );
    ...
    context("with a user that has been assigned to two workspaces", function() {
      it("returns true for both", function() {
        ...
        this.userSchedule.assignWorkspace(workspaces[2]);
        expect(
          this.userSchedule.hasUserWorkedOnWorkspace(workspaces[1]),
        ).to.equal(true);
        expect(
          this.userSchedule.hasUserWorkedOnWorkspace(workspaces[2]),
        ).to.equal(true);
